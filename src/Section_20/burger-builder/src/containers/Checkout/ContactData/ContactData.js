...
class ContactData extends Component {
    ...
    // HERE we now add additional improvements by using updateObect to update state
    inputChangedHandler = (event, inputIdentifier) => {
        const updatedFormElement = updateObject(this.state.orderForm[inputIdentifier], {
            value: event.target.value,
            // We need to change the arguments here to now reflect refactorization with updateObject
            valid: checkValidity(event.target.value, this.state.orderForm[inputIdentifier].validation),
            touched: true
        });
        // the new object is the new properties we are trying to change - which is the order input 
